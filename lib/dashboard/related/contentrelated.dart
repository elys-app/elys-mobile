...
class ContentRelatedPage extends StatefulWidget {
  ContentRelatedPage({Key? key, required Content this.item}) : super(key: key);

  final Content item;

  @override
  _ContentRelatedPageState createState() => _ContentRelatedPageState();
}

class _ContentRelatedPageState extends State<ContentRelatedPage> {
  ...
  Future<List<ListTile>> _getEventList() async {
    final result = await Amplify.DataStore.query(Event.classType,
        where: Event.CONTENTID.eq(widget.item.id));
        ...
        child: Column(
          ...
          children: <Widget>[
            Padding(
              padding: EdgeInsets.all(10),
              child: Text(
                'Events relate to the \'${widget.item.description}\' media item',
